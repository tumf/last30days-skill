...
- [ ] Add `FIRECRAWL_API_KEY` to config loading and backend detection paths in `scripts/lib/env.py`, `scripts/lib/pipeline.py`, `scripts/lib/resolve.py`, and `scripts/lib/setup_wizard.py`, keeping the behavior aligned with the `configuration-and-setup` and `cli-runtime` canonical spec domains. (verification: unit - tests/test_resolve.py tests/test_setup_openclaw.py)
